...
import { Mail, ExternalLink, Shield, Clock, Send, CheckCircle, MapPin, MessageSquare, Phone, ChevronDown } from 'lucide-react'
...
function ThemedSelect({ label, name, value, onChange, options, placeholder = 'Select...' }) {
  ...
  return (
    <div className="flex flex-col gap-1.5 relative" ref={ref}>
      ...
      <button
        ...
        className="rounded-xl px-4 py-3 text-[14px] outline-none transition-all duration-200 w-full text-left flex items-center justify-between gap-3"
        style={{
          ...
          color: isSelected ? '#1e1b4b' : 'rgba(60,50,120,0.40)',
          boxShadow: focused || open ? '0 0 0 3px rgba(138,99,255,0.08)' : 'none',
        }}
      >
      ...
      <AnimatePresence>
        {open && (
          <motion.ul
            ...
            className="absolute left-0 right-0 z-50 rounded-xl py-2 max-h-56 overflow-y-auto"
            style={{
              top: '100%',
              marginTop: '4px',
              background: 'rgba(255,255,255,0.98)',
              border: '1px solid rgba(138,99,255,0.20)',
              boxShadow: '0 12px 40px rgba(109,86,165,0.18), 0 4px 12px rgba(109,86,165,0.10)',
          ...
          >
            {options.map((opt) => (
              <motion.li
                ...
                className="px-4 py-2.5 text-[14px] cursor-pointer transition-all duration-150 flex items-center gap-2.5"
                style={{
                  color: value === opt ? '#8A63FF' : '#1e1b4b',
                  background: value === opt ? 'rgba(138,99,255,0.08)' : 'transparent',
                  fontWeight: value === opt ? 600 : 400,
                }}
                whileHover={{ background: 'rgba(138,99,255,0.06)' }}
              >
                {/* Checkmark for selected */}
                <span className="w-4 h-4 flex items-center justify-center flex-shrink-0">
                  {value === opt && (
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#8A63FF" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
    ...
    try {
      const res = await fetch('https://api.web3forms.com/submit', {
        ...
        body: JSON.stringify({
          ...
          subject: `[Aqua Secure] ${form.topic}${
            form.category ? ` — ${form.category}: ${form.item}` : ''
          } — from ${form.name}`,
      ...
      >
        ...
        <div className="wrap relative z-10 text-center">
          ...
          >
            ...
            >
              Have questions, need a demo, or want to discuss your organisation's
              security needs? We'd love to hear from you.
            </motion.p>
          </motion.div>
        </div>
      </section>
                    ...
                    >
                      ...
                      <form onSubmit={submit} className="flex flex-col gap-5">
                        ...
                        <div className="flex flex-col gap-3">
                          ...
                          {/* ── Second Dropdown (Products/Solutions) ── */}
                          <AnimatePresence>
                            {form.category && (
                              <motion.div
                              ...
                              >
                                <ThemedSelect
                                  ...
                                  placeholder={`— Select a ${form.category.slice(0, -1)} —`}
                                />
              ...
              {INFO_CARDS.map((card, i) => {
                ...
                return (
                  <motion.div
                    ...
                    onMouseLeave={(e) => {
                      e.currentTarget.style.borderColor = 'rgba(109,86,165,0.18)'
